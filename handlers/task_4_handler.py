...
def check_id_in_list(user_id):
    return user_id in users_current_task_4_words
...
def create_new_words_for_new_user(user_id):
    ...
    users_current_task_4_words[user_id] = {'correct_word': correct_word, 'incorrect_word': incorrect_word,
                                           'current_num': 0}


def get_words_user(user_id):
    return [users_current_task_4_words[user_id]['correct_word'], users_current_task_4_words[user_id]['incorrect_word']]
...
def check_correct_word(user_id, message):
    return message == users_current_task_4_words[user_id]['correct_word']
